...
IDALIB_PATH = Path(
    "/Applications/IDA Professional 9.1.app/Contents/MacOS/idalib/python/"
)
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python test_plugin.py dist/basic_ida_plugin-0.1.0-py3-none-any.whl"
        )
        sys.exit(1)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        run_command(
            ["uv", "venv", "--no-project", "--seed", str(venv_path)], show_output=False
        )
        ...
        run_command(
            [str(pip_executable), "install", str(IDALIB_PATH)], show_output=False
        )
        ...
        if not plugin_manager_wheels:
            print(
                "Error: idapro_plugin_manager wheel not found in dist/. Please build it first (e.g., by running 'python -m build')."
            )
            sys.exit(1)
        assert len(plugin_manager_wheels) == 1
        plugin_manager_wheel = plugin_manager_wheels[0]
        run_command(
            [str(pip_executable), "install", str(plugin_manager_wheel)],
            show_output=False,
        )
        ...
        test_dll_source_path = (
            PROJ_ROOT / "tests" / "data" / "Practical Malware Analysis Lab 01-01.dll_"
        )
        ...
        if "Library not loaded" in result.stderr and "PyQt5/QtWidgets" in result.stderr:
            print(
                "WARNING: plugin requires Qt GUI library that we can't test here in a headless environment"
            )
